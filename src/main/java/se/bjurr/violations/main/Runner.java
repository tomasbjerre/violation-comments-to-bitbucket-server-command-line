...
public class Runner {
	...
	public void main(final String args[]) throws Exception {
		...
		final String parsersString = Arrays.asList(Parser.values()).stream()
				.map((it) -> it.toString()).collect(Collectors.joining(", "));
		final Argument<List<List<String>>> violationsArg = stringArgument(
				"--violations", "-v")
				.arity(4)
				.repeated()
				.description(
				...
				.build();
		final Argument<SEVERITY> minSeverityArg = enumArgument(SEVERITY.class,
				"-severity", "-s").defaultValue(INFO)
				.description("Minimum severity level to report.").build();
		final Argument<Boolean> showDebugInfoArg = optionArgument(
				"-show-debug-info")
				.description(
						"Please run your command with this parameter and supply output when reporting bugs.")
				.build();

		final Argument<Boolean> commentOnlyChangedContentArg = booleanArgument(
				"-comment-only-changed-content", "-cocc").defaultValue(true)
				.build();
		final Argument<Boolean> commentOnlyChangedFilesArg = booleanArgument(
				"-comment-only-changed-files", "-cocf")
				.defaultValue(true)
				.description(
						"True if only changed files should be commented. False if all findings should be commented.")
				.build();
		final Argument<Boolean> createCommentWithAllSingleFileCommentsArg = booleanArgument(
				"-create-comment-with-all-single-file-comments", "-ccwasfc")
				.defaultValue(false).build();
		final Argument<Boolean> createSingleFileCommentsArg = booleanArgument(
				"-create-single-file-comments", "-csfc").defaultValue(true)
				.build();
		final Argument<Boolean> keepOldCommentsArg = booleanArgument(
				"-keep-old-comments").defaultValue(false).build();
		final Argument<String> commentTemplateArg = stringArgument(
				"-comment-template")
				.defaultValue("")
				.description(
						"https://github.com/tomasbjerre/violation-comments-lib")
				.build();
		final Argument<Integer> pullRequestIdArg = integerArgument(
				"-pull-request-id", "-prid").required().build();
		final Argument<String> projectKeyArg = stringArgument("-project-key",
				"-pk").required().build();
		final Argument<String> repoSlugArg = stringArgument("-repo-slug", "-rs")
				.required().build();
		final Argument<String> bitbucketServerUrlArg = stringArgument(
				"-server-url", "-url").required().build();
		final Argument<String> proxyHostArg = stringArgument("-proxy-host")
				.defaultValue("").build();
		final Argument<Integer> proxyPortArg = integerArgument("-proxy-port")
				.defaultValue(0).build();
		final Argument<String> proxyUserArg = stringArgument("-proxy-user")
				.defaultValue("").build();
		final Argument<String> proxyPassArg = stringArgument("-proxy-password")
				.defaultValue("").build();
		final Argument<String> usernameArg = stringArgument("-username")
				.defaultValue("").build();
		final Argument<String> passwordArg = stringArgument("-password")
				.defaultValue("").build();
		final Argument<String> personalAccessTokenArg = stringArgument(
				"-personal-access-token", "-pat").defaultValue("").build();
		final Argument<Boolean> createSingleFileCommentsTasksArg = booleanArgument(
				"-create-single-file-comments-tasks", "-csfct").defaultValue(
				false).build();
		final Argument<Integer> commentOnlyChangedContentContextArg = integerArgument(
				"-comment-only-changed-content-context", "-coccc")
				.defaultValue(5).build();
		final Argument<Integer> maxNumberOfViolationsArg = integerArgument(
				"-max-number-of-violations", "-max").defaultValue(
		...
		try {
			...
			this.commentOnlyChangedContent = parsed
					.get(commentOnlyChangedContentArg);
			this.commentOnlyChangedFiles = parsed
					.get(commentOnlyChangedFilesArg);
			this.createCommentWithAllSingleFileComments = parsed
					.get(createCommentWithAllSingleFileCommentsArg);
			this.createSingleFileComments = parsed
					.get(createSingleFileCommentsArg);
			...
			this.createSingleFileCommentsTasks = parsed
					.get(createSingleFileCommentsTasksArg);
			this.commentOnlyChangedContentContext = parsed
					.get(commentOnlyChangedContentContextArg);
		...
		for (final List<String> configuredViolation : this.violations) {
			final String reporter = configuredViolation.size() >= 4 ? configuredViolation
					.get(3) : null;
			...
			if (this.minSeverity != null) {
				allParsedViolations = Filtering.withAtLEastSeverity(
						allParsedViolations, this.minSeverity);
			}
			allParsedViolations.addAll(parsedViolations);
		}

		System.out.println("PR: " + this.projectKey + "/" + this.repoSlug + "/"
				+ this.pullRequestId + " on " + this.bitbucketServerUrl);
		final ViolationCommentsToBitbucketServerApi violationCommentsToBitbucketServerApi = violationCommentsToBitbucketServerApi();
		...
		try {
			...
			violationCommentsToBitbucketServerApi //
					...
					.withViolationsLogger(new ViolationsLogger() {
						@Override
						public void log(final Level level, final String string) {
							if (level != Level.FINE
									|| Runner.this.showDebugInfo) {
	...
	public String toString() {
		return "Runner [violations=" + this.violations
				+ ", commentOnlyChangedContent="
				+ this.commentOnlyChangedContent
				+ ", createCommentWithAllSingleFileComments="
				+ this.createCommentWithAllSingleFileComments
				+ ", createSingleFileComments=" + this.createSingleFileComments
				+ ", minSeverity=" + this.minSeverity + ", keepOldComments="
				+ this.keepOldComments + ", commentTemplate="
				+ this.commentTemplate + ", pullRequestId="
				+ this.pullRequestId + ", projectKey=" + this.projectKey
				+ ", repoSlug=" + this.repoSlug + ", bitbucketServerUrl="
				+ this.bitbucketServerUrl + ", proxyHost=" + this.proxyHost
				+ ", proxyPort=" + this.proxyPort + ", proxyUser="
				+ this.proxyUser + ", proxyPass=" + this.proxyPass
				...
				+ this.commentOnlyChangedContentContext
				+ ", maxNumberOfViolations=" + this.maxNumberOfViolations + "]";
	}
}
